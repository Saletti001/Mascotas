// =========================================
// ColiseumUI.js - VISTA (CSS, DOM y Animaciones)
// =========================================
...
window.ColiseumUI = {
    ...
    configurarDOM: function() {
        ...
        // Buscar el título y meterlo en la caja. Súper restrictivo para evitar agarrar el del Arcade.
        ...
        if (btnLeave && currentScreen) {
            ...
            currentScreen.appendChild(btnLeave); // Fuera de la caja oscura
        }
    },

    actualizarGraficos: function(p, e) {
        // FORZAR ESCRITURA DE NOMBRES Y RAREZAS
        let pNameEl = document.querySelector(".fighter-left .fighter-name") || document.getElementById("battle-player-name");
        if (pNameEl) pNameEl.innerHTML = `<strong>${p.nombre}</strong><br><span style="color:#aaa; font-size:10px; font-weight:normal;">(Nv. ${p.adn.level || 1})</span>`;
        
        let eNameEl = document.querySelector(".fighter-right .fighter-name") || document.getElementById("battle-enemy-name");
        if (eNameEl) eNameEl.innerHTML = `<strong>${e.nombre}</strong><br><span style="color:#aaa; font-size:10px; font-weight:normal;">(${e.rareza} - ${e.element})</span>`;

        // INYECCIÓN DE SVG
        let pVisual = document.querySelector(".fighter-left .fighter-sprite") || document.getElementById("player-visual-box");
        let eVisual = document.querySelector(".fighter-right .fighter-sprite") || document.getElementById("enemy-visual-box");
    ...
    actualizarHP: function(p, e) {
        ...
        let pBar = document.querySelector(".fighter-left [class*='hp-bar-fill']") || document.getElementById("player-hp-bar");
        let eBar = document.querySelector(".fighter-right [class*='hp-bar-fill']") || document.getElementById("enemy-hp-bar");
        if(pBar) pBar.style.width = `${pctP}%`;
        if(eBar) eBar.style.width = `${pctE}%`;

        let pTxt = document.querySelector(".fighter-left .hp-text") || document.getElementById("player-hp-text");
        let eTxt = document.querySelector(".fighter-right .hp-text") || document.getElementById("enemy-hp-text");
        if(pTxt) pTxt.innerText = `${Math.floor(p.hp)} / ${p.maxHp}`;
        if(eTxt) eTxt.innerText = `${Math.floor(e.hp)} / ${e.maxHp}`;

        let pSide = document.querySelector(".fighter-left") || document.getElementById("player-sprite-battle");
        let eSide = document.querySelector(".fighter-right") || document.getElementById("enemy-sprite-battle");
    ...
    agregarLog: function(texto) {
        const logBox = document.querySelector(".battle-log-container") || document.getElementById("battle-log");
        if (logBox) {
    ...
    animarAtaque: function(esJugador) {
        const el = esJugador ? (document.querySelector(".fighter-left .fighter-sprite") || document.getElementById("player-visual-box")) : (document.querySelector(".fighter-right .fighter-sprite") || document.getElementById("enemy-visual-box"));
        if(el) {
    ...
    animarDano: function(esJugador) {
        ...
        if(area) {
            ...
            setTimeout(() => area.classList.remove("shake-effect"), 400);
        }
    },

    mostrarTextoFlotante: function(esJugador, texto, claseAdicional) {
        const sideEl = esJugador ? (document.querySelector(".fighter-left") || document.getElementById("player-sprite-battle")) : (document.querySelector(".fighter-right") || document.getElementById("enemy-sprite-battle"));
        if(!sideEl) return;
