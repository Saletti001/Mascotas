// =========================================
// ColiseumUI.js - VISTA Y ANIMACIONES V9.15 (SISTEMA DE 4 BOTONES)
// =========================================
...
window.ColiseumUI = {
    inyectarCSS: function() {
        ...
        style.innerHTML = `
            /* ========================================= */
            /* ESTRUCTURA GLOBAL Y CAJA PRINCIPAL        */
            /* ========================================= */
            ...
            .hp-bar-fill-red, #enemy-hp-bar { background: linear-gradient(90deg, #ff6b6b, #d9534f) !important; height: 100%; border-radius: 6px; transition: width 0.3s;}
            .hp-text, #player-hp-text, #enemy-hp-text { font-size: 11px !important; color: #fff !important; font-weight: bold; margin-top: 4px !important; text-align: center; width: 100%; }
            ...
            .vs-badge-battle { position: relative !important; display: inline-block !important; font-size: 24px !important; font-weight: 900 !important; font-style: italic !important; color: #ffcc00 !important; z-index: 50 !important; margin: 0 !important; animation: vsPulse 1.5s infinite ease-in-out !important; }
            ...
            /* ========================================= */
            /* NUEVO: GRID PARA LOS 4 BOTONES DE ATAQUE  */
            /* ========================================= */
            #battle-controls, .controls-container { 
                width: 100% !important; 
                display: grid !important; 
                grid-template-columns: 1fr 1fr; /* 2 Botones por fila */
                gap: 8px !important; 
                margin-top: 15px !important; 
            }
            .battle-btn { padding: 12px 5px !important; border-radius: 8px !important; text-transform: uppercase !important; letter-spacing: 0.5px !important; transition: 0.2s !important; font-weight: 900 !important; font-size: 11px !important; color: white !important; cursor: pointer !important; border: 1px solid rgba(255,255,255,0.2) !important; text-shadow: 1px 1px 2px rgba(0,0,0,0.5) !important; display: flex; flex-direction: column; align-items: center; justify-content: center; line-height: 1.2; }
            .battle-btn span.uses { font-size: 9px; font-weight: normal; color: rgba(255,255,255,0.7); margin-top: 2px;}
            
            .slot-1 { background: linear-gradient(90deg, #ff5722, #d84315) !important; box-shadow: 0 4px 10px rgba(255, 87, 34, 0.3) !important; }
            .slot-2 { background: linear-gradient(90deg, #ff9800, #e65100) !important; box-shadow: 0 4px 10px rgba(255, 152, 0, 0.3) !important; }
            .slot-3 { background: linear-gradient(90deg, #009688, #00695c) !important; box-shadow: 0 4px 10px rgba(0, 150, 136, 0.3) !important; }
            .slot-4 { background: linear-gradient(90deg, #9c27b0, #4a148c) !important; box-shadow: 0 4px 15px rgba(156, 39, 176, 0.6) !important; border: 1px solid #e040fb !important;}
    ...
    configurarDOM: function() {
        ...
        if (controls) {
            ...
            // NUEVO: Ahora son 4 slots (el 4to empieza bloqueado)
            controls.innerHTML = `
                <button id="btn-slot-1" class="battle-btn slot-1">Ataque Básico</button>
                <button id="btn-slot-2" class="battle-btn slot-2">Ataque Especial</button>
                <button id="btn-slot-3" class="battle-btn slot-3">Táctica</button>
                <button id="btn-slot-4" class="battle-btn slot-4" disabled>🔒 Nv. 25+</button>
            `;
    ...
    actualizarBotonesSkills: function(jugador) {
        const ataques = jugador.ataquesEquipados;
        if (!ataques) return;

        for (let i = 0; i < 4; i++) {
            let btn = document.getElementById(`btn-slot-${i+1}`);
            if (btn && ataques[i]) {
                let usosText = ataques[i].usos === 99 ? "∞" : `${ataques[i].usosRestantes}/${ataques[i].usos}`;
                btn.innerHTML = `${ataques[i].nombre} <span class="uses">[${usosText}]</span>`;
                
                // Lógica de desbloqueo para el slot 4 (Definitivo en Nv 25)
                if (i === 3) {
                    if (jugador.adn.level < 25) {
                        btn.innerHTML = `🔒 Nv. 25+`;
                        btn.disabled = true;
                    ...
                    btn.disabled = (ataques[i].usosRestantes <= 0 && ataques[i].usos !== 99);
                }
            }
        }
    },
