// =========================================
// ShopManager.js - BAZAR, MATRIZ TÁCTICA Y PREMIUM (V9.8 - FIX CLONES Y CENTRADO)
// =========================================
    ...
    procesarCompra: function(item) {
        if (item.currency === "EV") {
            ...
            if (item.id === "bio_nucleo_basico") {
                ...
                // ✨ LECTURA DIRECTA DE TUS BASES DE DATOS V14 (genoma.js y caras.js)
                const formasBase = (window.GenomaBBDD && window.GenomaBBDD.cuerpo) ? window.GenomaBBDD.cuerpo : ["gota", "frijol", "circulo", "cuadrado", "triangulo"];
                const coloresBase = ["#ff6b6b", "#4dd0e1", "#fdfd96", "#b19cd9", "#77DD77", "#ff9800", "#ffb347", "#a8e6cf"];
                const elementosBase = (window.GenomaBBDD && window.GenomaBBDD.afinidad) ? window.GenomaBBDD.afinidad : ["Biomutante", "Viral", "Cibernético", "Radiactivo", "Tóxico", "Sintético"];
                ...
                // Asignaciones idénticas a app.js
                ...
                const eyeRandom = (window.GenomaBBDD && window.GenomaBBDD.ojos) ? window.GenomaBBDD.ojos[Math.floor(Math.random() * window.GenomaBBDD.ojos.length)] : obtenerClaveAleatoria(typeof dicOjos !== 'undefined' ? dicOjos : {});
                const mouthRandom = (window.GenomaBBDD && window.GenomaBBDD.boca) ? window.GenomaBBDD.boca[Math.floor(Math.random() * window.GenomaBBDD.boca.length)] : obtenerClaveAleatoria(typeof dicBocas !== 'undefined' ? dicBocas : {});
                ...
                // ✨ ESTRUCTURA EXACTA A "miPrimerGeno" (Sin variables basura que rompan el centrado)
                const huevo = {
                    ...
                    genes: {
                        ...
                        ojos: { dom: eyeRandom, rec: (window.GenomaBBDD && window.GenomaBBDD.ojos) ? window.GenomaBBDD.ojos[Math.floor(Math.random() * window.GenomaBBDD.ojos.length)] : obtenerClaveAleatoria(typeof dicOjos !== 'undefined' ? dicOjos : {}) },
                        boca: { dom: mouthRandom, rec: (window.GenomaBBDD && window.GenomaBBDD.boca) ? window.GenomaBBDD.boca[Math.floor(Math.random() * window.GenomaBBDD.boca.length)] : obtenerClaveAleatoria(typeof dicBocas !== 'undefined' ? dicBocas : {}) },
                        espalda: { dom: "ninguno", rec: "ninguno" },
                        cabeza: { dom: "ninguno", rec: "ninguno" },
                        afinidad: { dom: elementoRandom, rec: recElementoRandom } 
                    },
                    // Variables requeridas para la lógica de incubación
                    isEgg: true, 
                    incubating: false, 
                    hatchDuration: 60000, 
                    hatchTime: 0, 
